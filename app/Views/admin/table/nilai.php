...
<div class="row">
    <div class="col-md-12">
        ...
        <select class="form-control" name="siswa" id="gantisiswa">
            ...
            <?php
                foreach ($kelas->getResult() as $data){
                    $c = db_connect();
                    $kelas = $c->table("kelas")->where(["id_kelas" => $data->id_kelas])->get()->getRow()->nama_kelas;
                    echo "<optgroup label='Kelas $kelas'>";
                        $murid = $c->table("siswa")->where(["id_kelas" => $data->id_kelas])->get();
...
<script type="text/javascript">
    ...
    $("#gantisiswa").on("change", function(){
        ...
        $.ajax({
            url: "<?= base_url() ?>/admin/api/nilai/get_mapel/" + $("#gantisiswa").val(),
            type: "GET",
            success: function(x){
    ...
    $("#mapel").on("change", function(){
        $.ajax({
            async: false,
            url: "<?= base_url() ?>/admin/api/nilai/ganti_siswa/" + $("#gantisiswa").val() + "/" + $(this).val() ,
            type: "GET",
            success: function(c){
    ...
    function save(){
        ...
        $.ajax({
            url: "<?= base_url() ?>/admin/api/nilai/update",
            type: "POST",
            data: data,
            success: function(c){
                if (c['status'] == "success"){
